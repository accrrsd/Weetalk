...
import selectInputStyle from './profile-select.module.css'
...
type TProfileForm = {
  ...
  contactType: string | number | undefined
  previewFromApi: string | null
...
export const ProfileForm = ({ formHook, loader, submitSuccess, contactType, previewFromApi, onSubmitWrapper, setContactType }: TProfileForm) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitWrapper)} className={style.form}>
      <AddPhoto formHook={formHook} inputName="photo" previewImageUrl={previewFromApi} />
      ...
      <div className={style.submitWrapper}>
        {submitSuccess !== null && (
          <span className={submitSuccess ? style.submitSuccessColor : style.submitDenyColor}>
            {submitSuccess ? 'Карточка успешно обновлена' : 'Ошибка обновления карточки'}
          </span>
        )}
        <div className={`${style.submitLabelWrapper} ${loader ? style.submitLabelWrapperDisableEvents : ''}`}>
          <label htmlFor="submitButton" className={`${style.submit} ${loader ? style.submitButtonDisablePadding : ''}`}>
            ...
            <span className={style.submitContent}>{loader ? <Oval color="#7e7ee7" height={27} secondaryColor="#d9d9f8"></Oval> : 'Обновить'}</span>
          </label>
        </div>
      </div>
    </form>
