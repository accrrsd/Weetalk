...
export const AddPhoto = ({
  ...
  const base64Preview = `http://weetalk.online/api/v1/img/${previewImageBase64}`;
    ...
    <div className={style.wrapper}>
      <div
        className={`${style.image} ${
          photoChanged && onChangeStyle ? onChangeStyle : ''
        }`}
      ...
      {checkError(inputName, errors) && (
        <span className={style.error}>{checkError(inputName, errors)}</span>
      )}
    </div>
  );
};
